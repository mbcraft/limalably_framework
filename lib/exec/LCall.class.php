...
class LCall {
    ...
    private function executeRoute($call_spec, $all_param_data) {
        ...
        if ($all_param_data) {
            ...
            if (isset($all_param_data['parameters'])) {
                $parameters = $all_param_data['parameters'];
            } else {
                $parameters = [];
            }
            if (isset($all_param_data['capture'])) {
                $capture = $all_param_data['capture'];
            } else {
                $capture = [];
            }
            if (isset($all_param_data['input'])) {
                $input_view = $all_param_data['input'];
            } else {
                $input_view = new LTreeMap();
            }
            if (isset($all_param_data['session'])) {
    ...
    private function executeProcFile($call_spec, $all_param_data) {
        if (!$this->isValidProcFileRoute($call_spec))
            throw new \Exception("Unable to find valid proc : " . $call_spec);
        ...
        if ($all_param_data) {
            ...
            if (isset($all_param_data['rel_input'])) {
                $rel_input = $all_param_data['rel_input'];
            }
            if (isset($all_param_data['session'])) {
                $session = $all_param_data['session'];
            }
            if (isset($all_param_data['rel_session'])) {
                $rel_session = $all_param_data['rel_session'];
            }
            if (isset($all_param_data['context_path'])) {
    ...
    private function executeClassMethod($call_spec, $all_param_data) {
        ...
        if (LStringUtils::startsWith($call_spec, '\\'))
            $call_spec = substr($call_spec, 1);
        ...
        if (!$reflection_class->hasMethod($method_name))
            throw new \Exception("Unable to find method on class : " . $call_spec);
        $reflection_method = $reflection_class->getMethod($method_name);
        if ($reflection_method->isAbstract())
            throw new \Exception("Method to call is abstract and can't be called : " . $call_spec);
        if ($static_call && !$reflection_method->isStatic())
            throw new \Exception("Method to call is not static, use '" . self::OBJECT_METHOD_CALL_SEPARATOR . "' : " . $call_spec);
        if (!$static_call && $reflection_method->isStatic())
            throw new \Exception("Method to call is static, use '" . self::STATIC_METHOD_CALL_SEPARATOR . "' : " . $call_spec);
        ...
        foreach ($method_parameters as $mp) {
            ...
            foreach ($method_parameter_search_list as $method_search_spec) {
                ...
                switch ($method_search_spec) {
                    ...
                    case 'rel_input': {
                            if (isset($all_param_data['rel_input']) && $all_param_data['rel_input']->is_set($param_name)) {
                    ...
                    case 'rel_session': {
                            if (isset($all_param_data['rel_session']) && $all_param_data['rel_session']->is_set($param_name)) {
                    ...
                    case 'rel_output': {
                            if (isset($all_param_data['rel_output']) && $all_param_data['rel_output']->is_set($param_name)) {
