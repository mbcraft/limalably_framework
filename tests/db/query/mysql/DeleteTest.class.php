...
class DeleteTest extends LTestCase {
	...
	function testDeleteStatement() {
		...
		$d1 = delete('table_123')->where(_eq('field_name',12));
		$d2 = delete('table_123')->where(_eq('field_name',12))->inner_join(true,'table_for_join');
		$d3 = delete('table_123')->where(_eq('field_name',12))->inner_join(false,'table_for_join',_eq('field_j1',3));
