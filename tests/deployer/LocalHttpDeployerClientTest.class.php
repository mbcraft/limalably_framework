...
class LocalHttpDeployerClientTest extends LTestCase {
	...
	function testReset() {
		...
		$enemy_file = new LFile(SOFTWARE_DIR.'/DeployerTestLocalSite/enemy.txt');
	...
	function testTempClean() {
		...
		$other_dir = new LDir(SOFTWARE_DIR.'/DeployerTestLocalSite/other/');

		$other_dir->touch();

		$temp_dir = new LDir(SOFTWARE_DIR.'/DeployerTestLocalSite/temp/');

		$temp_dir->touch();

		$enemy_file = new LFile(SOFTWARE_DIR.'/DeployerTestLocalSite/temp/enemy.txt');
	...
	function testSetGetEnv() {
		...
		$df = new LFile(SOFTWARE_DIR.'/DeployerTestLocalSite/deployer.php');
	...
	function testProjectUpdate() {
		...
		$f1 = new LFile(SOFTWARE_DIR.'/DeployerTestLocalSite/project_file.txt');

		$this->assertTrue($f1->exists(),"Il file di progetto non è stato copiato con successo!");

		$f2 = new LFile(SOFTWARE_DIR.'/DeployerTestLocalSite/project_dir/my_file.txt');

		$this->assertTrue($f2->exists(),"Il file di progetto nella sottodirectory non è stato copiato con successo!");

		$f3 = new LFile(SOFTWARE_DIR.'/DeployerTestLocalSite/project_dir/subdir/my_subdir_file.txt');
	...
	function testDisappear() {
		...
		$fd = new LFile(SOFTWARE_DIR.'/DeployerTestLocalSite/deployer.php');
	...
	function testManualConfig() {
		...
		$host_config = new LFile(SOFTWARE_DIR.'/DeployerTestLocalSite/config/hostnames/my_host/config.json');

		$internal_config = new LFile(SOFTWARE_DIR.'/DeployerTestLocalSite/config/internal/framework.json');
	...
	function testFixPermissions() {
		...
		$this->assertTrue($this->isSuccess($r),"La procedura di fix permissions ha dato esito negativo! : ".$this->getErrorMessage($r));

		$this->assertEqual($p1->getPermissions(),$p_flags,"I flag non sono stati impostati correttamente!!");

		$dir_fix_permissions->delete(true);
