...
class RandomInDirTest extends LTestCase
{
    function testRandomFromFolderNoMtime()
    {
        ...
        $d = new LDir($_SERVER['FRAMEWORK_DIR']."tests/fs/test_dir/content_dir/");
        ...
        for($i=0;$i<30;$i++)
        {
            ...
            if ($result==$_SERVER['FRAMEWORK_DIR']."tests/fs/test_dir/content_dir/css_test.css") $css_count+=1;
            if ($result==$_SERVER['FRAMEWORK_DIR']."tests/fs/test_dir/content_dir/ext_test.plug.txt") $ext_count += 1;
            if ($result==$_SERVER['FRAMEWORK_DIR']."tests/fs/test_dir/content_dir/test_file.txt") $test_count += 1;
    ...
    function testRandomFromFolderWithFoldersNoMtime()
    {
        ...
        for($i=0;$i<30;$i++)
        {
            ...
            if ($result==$_SERVER['FRAMEWORK_DIR']."tests/fs/test_dir/content_dir/another_dir/") $another_count+=1;
            if ($result==$_SERVER['FRAMEWORK_DIR']."tests/fs/test_dir/content_dir/css_test.css") $css_count+=1;
            if ($result==$_SERVER['FRAMEWORK_DIR']."tests/fs/test_dir/content_dir/ext_test.plug.txt") $ext_count += 1;
            if ($result==$_SERVER['FRAMEWORK_DIR']."tests/fs/test_dir/content_dir/test_file.txt") $test_count += 1;
